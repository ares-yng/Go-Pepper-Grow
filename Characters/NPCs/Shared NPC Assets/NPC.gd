...
func setSpritesheet(filename): 
	var spritesheet = load(str("res://Characters/NPCs/npc_", getID(), "/", filename, ".tres"))
	$AnimatedSprite2D.set_sprite_frames(spritesheet)

func getNextDialogueID(currentSpecialDialogueCheckpoint): 
	if !data["is_current_special_dialogue_seen"] && data[str("special_dialogue_id", currentSpecialDialogueCheckpoint)] != null:
