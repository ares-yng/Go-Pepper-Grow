...
@onready var db = load("res://Game Systems/Resources/database_resource.gd").new()
...
func _process(delta):
	...
	if hotbarNode.updateHand: #change what player is holding
		if hotbarNode.newSelection: 
			...
			var equipmentFilename = hotbarItem["equipment_filename"]
			if equipmentFilename != null: #if there is an item and it has a hand animation
				var animSpr = hand.get_child(0)
				if animSpr.get_sprite_frames().get_name() != equipmentFilename: #if the equipment is different
					var reso = load(str("res://Characters/Player/Equipment System/Equipment/", equipmentFilename, ".tres"))
