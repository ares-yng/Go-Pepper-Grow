extends Node2D
@onready var db = preload("res://Game Systems/Resources/database_resource.gd").new()
...
func setupNewPlayer(new_player_id):
	...
	for i in allSceneData.size():
		if i == 0: #skip main_menu
			continue
		var scene = load(str("res://Maps/", allSceneData[i]["scene_path"], "/", allSceneData[i]["scene_filename"], ".tscn")).instantiate()
...
func loadFile(player_id = PlayerNode.playerID, doRefreshPlayer = true): #loads last saved information of given player id file
	...
	if doRefreshPlayer:
		...
		var newPlayer = load(str("res://Characters/Player/Player.tscn")).instantiate()
...
func openScene(scene_id, playerPos = 0, saveCurrent = true): #opens scene from load or changing maps
	...
	var newSceneNode = load(str("res://Maps/", newSceneData["scene_path"], "/", newSceneData["scene_filename"], ".tscn")).instantiate()
