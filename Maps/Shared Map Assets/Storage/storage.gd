...
func setData(saveData): 
	...
	for index in itemIDs.size(): #for each item slot, 
		var slot = load("res://Game Systems/Items/Slot.tscn").instantiate() #create a slot
		...
		if itemIDs[index] != null: #if there is an item in the slot, create an item with its quantity in the slot 
			var item = load("res://Game Systems/Items/Item.tscn").instantiate()
...
func setSpritesheet(filename):
	var spritesheet = load(str("res://Maps/Shared Map Assets/Storage/Storage Assets/", filename, ".tres"))
